...
namespace pmrrr { namespace lapack {
	...
	int odruv(int *iseed, int *n, FloatingType *x)
	{
		/* Initialized data */
	...
	/*        Convert 48-bit int to a real number in the interval (0,1) */
		...
		x[i__] = ((FloatingType) it1 + ((FloatingType) it2 + ((FloatingType) it3 + (
			FloatingType) it4 * 2.44140625e-4) * 2.44140625e-4) * 
