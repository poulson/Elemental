...
namespace pmrrr { namespace lapack {
	...
	int odrrf(int *n, FloatingType *d__, FloatingType *l, 
		FloatingType *ld, int *clstrt, int *clend, FloatingType *w, 
		FloatingType *wgap, FloatingType *werr, FloatingType *spdiam, FloatingType *
		clgapl, FloatingType *clgapr, FloatingType *pivmin, FloatingType *sigma, 
		FloatingType *dplus, FloatingType *lplus, FloatingType *work, int *info)
	{
		...
		/* Local variables */
		int i__;
		FloatingType s, bestshift, smlgrowth, eps, tmp, max1, max2, rrr1, rrr2, 
			znm2, growthbound, fail, fact, oldp;
		int indx;
		FloatingType prod;
		int ktry;
		FloatingType fail2, avgap, ldmax, rdmax;
		int shift;
	...
	//	    FloatingType *, int *);
		...
		FloatingType ldelta;
		int nofail;
		FloatingType mingap, lsigma, rdelta;
		//extern int odnan_(FloatingType *);
		int forcer;
		FloatingType rsigma, clwdth;
	...
	/*     Compute the average gap length of the cluster */
		clwdth = (d__1 = w[*clend] - w[*clstrt], fabs(d__1)) + werr[*clend] + werr[
			*clstrt];
		avgap = clwdth / (FloatingType) (*clend - *clstrt);
