...
#include <pmrrr/lapack/odrrd.hpp>
#include <pmrrr/lapack/odrrb.hpp>
#include <pmrrr/lapack/odrrc.hpp>
#include <pmrrr/lapack/odrrk.hpp>
#include <pmrrr/lapack/odrnv.hpp>
#include <pmrrr/lapack/odsq2.hpp>
...
namespace pmrrr { namespace lapack {
	...
	int odrre(const char *range, int *n, FloatingType *vl, 
		FloatingType *vu, int *il, int *iu, FloatingType *d__, FloatingType *e, 
		FloatingType *e2, FloatingType *rtol1, FloatingType *rtol2, FloatingType *spltol, 
		int *nsplit, int *isplit, int *m, FloatingType *w, 
		FloatingType *werr, FloatingType *wgap, int *iblock, int *indexw, 
		...
		iwork, int *info)
	{
		...
		/* Builtin functions */
		//    FloatingType sqrt(FloatingType), log(FloatingType);
		...
		FloatingType gu;
		int cnt;
		FloatingType eps, tau, tmp, rtl;
		int cnt1, cnt2;
		FloatingType tmp1, eabs;
		int iend, jblk;
		FloatingType eold;
		int indl;
		FloatingType dmax__, emax;
		int wend, idum, indu;
		FloatingType rtol;
		int iseed[4];
	...
	/*              Decide whether dqds or bisection is more efficient */
			usedqd = (FloatingType) mb > in * .5 && ! forceb;
			wend = wbegin + mb - 1;
	...
	/*           the matrix is definite and we need not retreat. */
			...
			avgap = (d__1 = clwdth / (FloatingType) (wend - wbegin), fabs(
				d__1));
			if (sgndef == 1.) {
	/* Computing MAX */
				d__1 = wgap[wbegin];
	...
	/*           done by bisection when computing the eigenvectors */
			rtol = log((FloatingType) in) * 4. * eps;
			j = ibegin;
			i__2 = in - 1;
			for (i__ = 1; i__ <= i__2; ++i__) {
