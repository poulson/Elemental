...
#include <pmrrr/lapack/odsq3.hpp>
...
namespace pmrrr { namespace lapack {
	...
	int odsq2(int *n, FloatingType *z__, int *info)
	{
		...
		/* Local variables */
		FloatingType d__, e, g;
		int k;
		FloatingType s, t;
		int i0, i4, n0;
		FloatingType dn;
		int pp;
		FloatingType dn1, dn2, dee, eps, tau, tol;
		int ipn4;
		FloatingType tol2;
		int ieee;
		int nbig;
		FloatingType dmin__, emin, emax;
		int kmin, ndiv, iter;
		FloatingType qmin, temp, qmax, zmax;
		int splt;
		FloatingType dmin1, dmin2;
		int nfail;
		FloatingType desig, trace, sigma;
		int iinfo, ttype;
		/* Subroutine */ /*extern  int odsq3_(int *, int *, FloatingType *, 
	...
	/*     Check whether the machine is IEEE conformable. */
		/* ieee = oienv_(&c__10, "ODSQ2", "N", &c__1, &c__2, &c__3, &c__4) == 1 && oienv_(&c__11, "ODSQ2", "N", &c__1, &c__2,  */
		/* 	     &c__3, &c__4) == 1; */
		ieee = 1;
	...
	/*     Store trace, sum(eigenvalues) and information on performance. */
		z__[(*n << 1) + 1] = trace;
		z__[(*n << 1) + 2] = e;
		z__[(*n << 1) + 3] = (FloatingType) iter;
	/* Computing 2nd power */
		i__1 = *n;
		z__[(*n << 1) + 4] = (FloatingType) ndiv / (FloatingType) (i__1 * i__1);
		z__[(*n << 1) + 5] = nfail * 100. / (FloatingType) iter;
		return 0;
